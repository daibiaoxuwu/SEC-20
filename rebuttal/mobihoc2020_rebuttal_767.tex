...
%\noindent Thanks for three reviewers' constructive and insightful comments.
...
\textbf{Response:} Indeed, we do not provide the proof details to reduct our problem to the MWCDS problem. For simplicity, we present the specific definition of our problem, then use an illustrative example to show our problem is kind of MWCDS problem. We can add the details in the next version.
...
\textbf{Response:} 
%下面这句话是不是可以不说？直接解释我们的算法就行？
%We fully understand your points about our algorithm descriptions. 
The basic structure of our algorithm is similar with that of distance vector routing algorithm which adopts Bellman-Ford equation to update routing entries, but a node needs to maintain two network metrics (e.g., EBQ and PBT). We split the whole algorithm to three parts to make each of them short. Algorithm 1 summarizes the whole algorithm operated by a node in a distributed manner, then Algorithm 2 and 3 present the strategies to update the node's EBQ and PBT metrics according to the new received neighbors' metrics.
...
% 不知道reviewer是不是准确知道tail length含义？
\textbf{Response:} That is true. It would be more comprehensive to evaluate our protocol with more indicators such as the number of concurrent senders, the number of retransmissions, the tail length of concurrent transmission.

\textbf{(3.2)} In the first paragraph of Section 3.2, the sentences related to EBQ may be not very accurate. EBQ is related to both variables. %i.e., expected time of preamble packet broadcast and number of children. 
One must be controlled for the other variable.

\textbf{Response:} We agree with your comments. We can modify the sentences to remove the ambiguity.
