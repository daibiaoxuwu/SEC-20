...
module ChaseTestC {
  ...
  message_t tree_serial_pkt;
  bool tree_send_lock = FALSE;
#endif
  ...
  event void TreeInit.startDone() {
#ifndef ENABLE_PR
    //if (call TreeInit.isSender()) {
        tree_beacon_msg_t* ptr = (tree_beacon_msg_t*)call treeSerialPkt.getPayload(&tree_serial_pkt, sizeof(tree_beacon_msg_t));

        if ((ptr == NULL) || tree_send_lock) { return; }

        ptr->id = TOS_NODE_ID;
        ptr->ebw = call TreeInfo.getEBW();
        ptr->path_ebw = call TreeInfo.getPathEBW();
        ptr->elder_id = call TreeInfo.getElderID();
        // ptr->delay = call TreeInfo.getDelay();
        ptr->delay = call TreeInfo.getWeight();
        ptr->children_num = call TreeInfo.getChildrenNum();
        ...
        if (call treeSerialSend.send(AM_BROADCAST_ADDR, &tree_serial_pkt, sizeof(tree_beacon_msg_t)) == SUCCESS) {
            tree_send_lock = TRUE;
        }
    //}
#endif
  ...
  event void ChaseControl.dc_started(error_t err) {
    if (err == SUCCESS) {
      // choose sink node
      if (TOS_NODE_ID == 3)
        call floodTimer.startOneShot(120*1024UL);
  ...
  event void floodTimer.fired() {
    ...
    call floodTimer.startOneShot(20480);
  ...
  event void ChaseDelay.getDelay(chase_delay_t* lay) {
    ...
    pr("delay %u %u %u %u %u %u %u %u %u %u %u %u %u\n", lay->seq_no, lay->last_hop_id, time_rec_h, time_rec_l, delay_h, delay_l, lay->parent_ETD, lay->local_ETD, lay->delta_ETD, lay->long_link_sender, lay->MPD, lay->shortcut_sender, lay->tree_sender);
#ifndef ENABLE_PR
      {
          ...
          p->parent_ETD = lay->parent_ETD;
          p->local_ETD = lay->local_ETD;
          p->delta_ETD = lay->delta_ETD;
          p->long_link_sender = lay->long_link_sender;
          p->MPD = lay->MPD;
          p->shortcut_sender = lay->shortcut_sender;
          p->tree_sender = lay->tree_sender;
  ...
  event void treeSerialSend.sendDone(message_t* pkt, error_t error) {
      if (&tree_serial_pkt == pkt) {
          tree_send_lock = FALSE;
      }
      // call ChaseControl.dc_start();
  }
#endif
