...
\begin{table}[t]
  ...
  \begin{tabular}{ccccc}
  \hline
   Reference & Scenarios   & Accuracy & \begin{tabular}[c]{@{}l@{}}Quantitative \\ Model\end{tabular}& Distance$^{\mathrm{a}}$ \\
  \hline
  Eiband et al.~\cite{eiband2017understanding}  & naked eye & no & no & 1 meter \\
  Kwon et al.~\cite{kwon2013covert}  & naked eye & no & no &-$^{\mathrm{b}}$\\
Schaub et al.~\cite{schaub2012password} & naked eye & no & no & -\\
  Maggi et al.~\cite{maggi2011poster}  & camera & yes  & no &- \\
  \hline
  \hline
  \textbf{\textsf{SRPeek}} & COTS phones & yes & yes  & 1.8 / 6 meter\\
  \hline
  \multicolumn{5}{l}{$^{\mathrm{a}}$The maximum distance to the victim's screen.}\\

  \multicolumn{5}{l}{$^{\mathrm{b}}$Distance is uncontrolled; The attacker stands next to or behind the victim.}\\
  \end{tabular}
