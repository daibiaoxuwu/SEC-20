...
while (<fin>) {
    ...
    if (@is_record[$nodeid] == 0) {
        @base_radio_on_time[$nodeid] = @log[1] * 65536 + @log[2];
        @base_time[$nodeid] = @log[3] * 65536 + @log[4];
        @is_record[$nodeid] = 1;
    } else {
        $total_time = @log[1] * 65536 + @log[2] - @base_radio_on_time[$nodeid];
        $radio_on = @log[3] * 65536 + @log[4] - @base_time[$nodeid];
...
for ($i = 0; $i < 40; $i++) {
	...
	if (@count[$i] == 41 && @com_time[$i] < 10000) {
