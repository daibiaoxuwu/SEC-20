...
for ($i = 0; $i < 50; $i++) {
	...
	while (<fin>) {
      ...
      if (@log[0] eq "tree") {
        ...
        if (@has_delay[$i] == 1 && @is_record[$i] == 0) {
            @base_radio_on_time[$i] = @log[3]*65535 + @log[4];
            @base_time[$i] = @log[1]*65535 + @log[2];
            @is_record[$i] = 1;
        } else {
          $radio_on_time = @log[3]*65535 + @log[4];
          $time_now = @log[1]*65535 + @log[2];
          @duty_cycle[$i] = ($radio_on_time - @base_radio_on_time[$i]) / ($time_now - @base_time[$i]);
        }
      }
    }
    close fin;
